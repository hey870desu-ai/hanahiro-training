#!/usr/bin/env python3
"""poem de reha 安積店 運営規程（地域密着型通所介護・3時間以上4時間未満）の Word 文書を生成。
...
def main():
    ...
    # =============== 表紙的タイトル ===============
    add_heading_centered(doc, 'poem de reha 安積店', size=14, after=4)
    add_heading_centered(doc, '運営規程', size=20, after=24)

    # =============== 第1条 事業の目的 ===============
    add_article(doc, 1, '事業の目的', [
        '株式会社はなひろ（以下「事業者」という。）が開設する poem de reha 安積店（以下「事業所」という。）が行う指定地域密着型通所介護（以下「通所介護」という。）の事業（以下「本事業」という。）の適正な運営を確保するために、人員及び管理運営に関する事項を定め、要介護状態にある高齢者及び介護予防・日常生活支援総合事業の対象者に対し、適正な通所介護を提供することを目的とする。',
    ])
    ...
    add_article(doc, 8, '通常の事業の実施地域', [
        '通常の事業の実施地域は、提供するサービスの種類に応じて次のとおりとする。',
    ])
    add_paren_list(doc, [
        '指定地域密着型通所介護　福島県郡山市の区域内（原則として郡山市内に住所を有する被保険者を対象とする）',
        '介護予防・日常生活支援総合事業（第1号通所事業）　福島県郡山市及び福島県須賀川市の区域内',
    ])
    p = doc.add_paragraph()
    p.paragraph_format.first_line_indent = Mm(4)
    p.paragraph_format.space_after = Pt(4)
    r = p.add_run('2　地域密着型通所介護について、市外の被保険者に対するサービス提供を行う場合には、関係市町村との協議に基づき郡山市長の同意を得た場合に限り行うものとする。')
    set_japanese_font(r, size=10.5)
    p = doc.add_paragraph()
    p.paragraph_format.first_line_indent = Mm(4)
    p.paragraph_format.space_after = Pt(4)
    r = p.add_run('3　前各項の地域を超えて行う送迎については、事業者と利用者との協議により定める。')
    set_japanese_font(r, size=10.5)
